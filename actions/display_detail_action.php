...
//         $productPrice = $row["price"];
//         $imagePath = $row["image_path"];
//         $quantity = isset($row["cart_quantity"]) ? $row["cart_quantity"] : 0; // Set quantity to 0 if not found in cart
...
//     // Prepare and execute SQL query to fetch product details by product ID
//     $sql = "SELECT p.*, c.quantity AS cart_quantity FROM Product p LEFT JOIN Carts c ON p.product_id = c.product_id WHERE p.product_id = $productId";
//     $result = $conn->query($sql);
...
//         echo '</div>';
//         // echo '<input type="text" class="form-control bg-secondary border-0 text-center" value="' . $quantity . '">';
//         echo '<input type="text" class="form-control bg-secondary border-0 text-center">';
//         echo '<div class="input-group-btn">';
//         echo '<form action="../actions/increase_quantity_action.php" method="post">';
//         // echo '<input type="hidden" name="condition" value="' . $condition . '">';
//         echo '<button class="btn btn-primary btn-plus" type="submit" name="product_id" value="' . $productId . '">';
...
// } else {
//     // Redirect back to the search page if product_id is not provided
